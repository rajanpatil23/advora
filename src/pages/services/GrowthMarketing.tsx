...
const data = serviceDetails["growth-marketing"];
...
const GrowthMarketing = () => {
  ...
  const features = [
    ...
    { icon: BarChart3, title: "CRO", description: "Continuous conversion rate optimization." },
    { icon: Users, title: "Lead Generation", description: "Scalable lead acquisition systems." },
  ];
  ...
  return (
    <div className="min-h-screen bg-background animate-fade-in">
      ...
      <main>
            ...
            <div className="rounded-[2.5rem] sm:rounded-[3rem] bg-primary/10 dark:bg-card p-4 sm:p-8 md:p-12">
              <div className="grid lg:grid-cols-2 gap-6 sm:gap-8 lg:gap-12 items-center">
                <div className="rounded-xl sm:rounded-2xl overflow-hidden">
                  <img src={heroDashboard} alt="Growth Marketing" className="w-full h-auto object-cover aspect-[4/3]" />
                </div>
                  ...
                  <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 pt-2">
                    <Link to="/contact" className="w-full sm:w-auto">
                      <Button className="bg-foreground text-background hover:bg-foreground/90 rounded-full px-6 py-5 sm:px-8 sm:py-6 font-medium w-full sm:w-auto">
                        Drive Growth <ArrowRight className="ml-2 h-4 w-4" />
                      </Button>
                    </Link>
        ...
        <ServiceReviewsSection reviews={data.reviews} serviceName="growth marketing" />
        <ServiceProcessSection {...data.process} />
        <ServiceToolStackSection {...data.toolStack} />

        <CTASection title="Ready to Accelerate Your Growth?" description="Let's build your acquisition engine together." buttonText="Get Started" />
      </main>
      <Footer />
    </div>
  );
